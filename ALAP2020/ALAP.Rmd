...
Kin count estimation has a long and distinguished pedigree in mathematical demography, starting with the work of @lotka1931orphanhood on modelling orphanhood in theorical populations across demographic regimes.
...
This is evidenced by the omnipresensce of the 'net reproductive rate' in accounts of human and non-human populations. 
...
These assumptions are often untested given the data sparcity [@livi_bacci_concise_1997;@volk_infant_2013]. 
...
With increasing periods of generational overlapp, individuals find themselves 'sandwiched' between aging parents and young children requiring their simultaneous attention and care [@daatland_norwegian_2010].
...
Losing an only child may become a more common experience in the context of global fertility decline, something particularly worrying for parents suddendly rendered childless.
...
How can we characterize the timing of offspring mortality as experienced by a mother using a formal demographic perspective? 
How do changes in mortality affect the availability of offspring over age from the point of view of a prospective mother?
In this paper, we uncover formal relationships to answer these questions and provide numerical applications using data from countries in Latin America during the second half of the twentieth century. Please see the [data section](#Data_appendix) in the Appendix for details on the data and analytical procedures.


## Formulas

$CS_a$ is the expected number of surviving children to a mother^[We´ll refer to mothers, assuming that fertility rates applies to all woman, meaning that all of them are exposed.] alive at aged $a$ in a female stable population with fertility rates $m_{x}$, mortality hazard $\mu_{x}$ and survival function $l_x=e^{-\int_{0}^{x}{\mu_t}\,dt}$ ($l_0=1$ during this work), as proposed by @goodman1974:
...
```{r plot_CS, fig.cap="Child Survival and Child survival related to cumulated fertility, for women aged 30, 40 and 50. Latinamerican countries in period 1950-2015"}
...
Following @KeyfitzMath work about living mothers count, we can consider $a>=\beta$ and assume that fertility pattern by age is concentrated around $\kappa$ (mean age at childbearing), and cumulated fertility (or gross reproduction rate because of female model) is $F_a =  \int_{\alpha}^a{m_{x}dx}$. An approximation of $l_x$ using Taylor´s theorem until second order around $\kappa$ would be:
...
Where $d_t$ is the death distribution from birth, $e_{0|a-x-t}$ is the life expectancy at birth until age $a-x-t$ and $e_{0|a-x}^\dagger$ the temporary dispertion measure. But most interesting would be to compare these years with the time that mothers would expect to live with their daughters. We call this the *Intensity Time in Bereavement* ($ITB$)): a ratio between expected time with a "lost" and expected time with a "life", that allows to make comparisons between population regimes.

$$ITL_a = \frac{\int_0^a{m_x e_{0|{a-x}}^\dagger dx}}{\int_0^a{m_x e_{0|a-x} dx}}$$

This is a ratio between child-years in two radically different states. Looks similar to the trascendental entropy measure $H$ (@KeyfitzMath) but considering all the cohorts born during the the mother´s life, weighted by their relative size $m_x$. In the figure \@ref(fig:plot_ITL_MAL) is shown that *ITB* is bigger for youg womans because of the weight of infant mortality in their mother experience, also with more dispersion for same levels of parity. Assuming constant period rates the expected time in years that a mother aged 30 passed with a death son was around 4% in some countries at middle XX Century. When increasing age, the survival experience depends less on infant mortality, and the distribution is around less than 2% for womans aged 50, converging to 0 on time. As extreme cases, in 1950-1955 Haiti women aged 30 would have experienced an intensity of `r round(CS_outs$ITL[CS_outs$name=="Haiti" & CS_outs$Age==30 & CS_outs$Period=="1950-1955"],1)`%, and in 2010-2015 `r round(CS_outs$ITL[CS_outs$name=="Haiti" &CS_outs$Age==30 & CS_outs$Period=="2010-2015"],1)`%, while the women of Costa Rica `r round(CS_outs$ITL[CS_outs$name=="Costa Rica" & CS_outs$Age==30 & CS_outs$Period=="1950-1955"],1)`%, and in 2010-2015 `r round(CS_outs$ITL[CS_outs$name=="Costa Rica" &CS_outs$Age==30 & CS_outs$Period=="2010-2015"],1)`%.
 
Another important factor for the child survival experience of mothers, is the mean age at child loss, called here $MAL$. This relation can be derived by starting with the mother age $x+t$ at each death child age $t$ at death, weighted by the fertility and survival function. In it, $\kappa$ is the mean age at childbirth for womens aged $a$, $MAD_{a-x}$ refers to the mean age at death for newborns that die before $a-x$, $F_a$ is the cumulated fertility for a women aged $a$:
...
For those latinamerican mothers aged 50 with 3 daughters born who suffered a lost, theyy experienced that at age 30 in average (figure \@ref(fig:plot_ITL_MAL)) (More can be analyzed here).
...
ITL <- ggplot(CS_outs)+  
                    geom_smooth(method="lm", aes(x=Fa, y=ITL, color=factor(Age)), 
                                se = F, formula= (y ~ exp(x)))+
...
We now consider the consequences of an absolute change $\delta$ in mortality in the range $[0,a-\alpha]$, where $\alpha$ is the start age of fertility risk. Considering that $m_{x,\delta}=m_{x}+\delta$ (@wrycza2012), and $l_{a-x}^\delta = e^{-\int_{0}^{a-x}{(\mu_t+\delta})dt}$:
...
That an equal change in mortality could happens at all ages is unlikelihood. As a [corollary](#Corollary) we inspect the case thtat only infant mortality is affected.

The relationship is less clear for a proportional change given the interaction of birth and mortality rates (@KeyfitzMath). Considering that $\mu_{x,\delta}=\mu_{x}(1+\delta)$ and $l_{a-x}^\delta = e^{-\int_{0}^{a-x}{\mu_t(1+\delta})dt}=(l_{a-x})^{(1+\delta)}$:
...
Dividing by $CS_a$ and multiplying by $\delta$ get one useful expression. This is the negative of cumulative hazard $H_a$ but considering a factor $0\leq \frac{CS_{a-t}}{CS_a}\leq1$ that takes in acccount the amount of survinvig descendatns that would be lost for a mother aged *a* in each age x of risk. 

$$\frac{\Delta CS_{a}}{CS_a} = - \left[\int_{0}^{a}{\mu_t  \, \frac{CS_{a-t}}{CS_a} \,dt \,}\right] \Delta \delta$$

This factor has a S-shape because $l_x$ curvature fertility acumulation, that gives more weight to first ages, and which is grater than 1 only for ages *a* with complete reproductive life and high infant mortality (with no aditional childs and higher risk for one age additional of survive).  
...
In a model with heterogeneity, determined at born with a multiplicative effect, $CS_a$ could be interpreted as a conditional expectation with random variables *K* for fertility heterogeneity and *Z* for mortality frailty.

$$CS_a(k,z) = \int_{0}^{\infty}\,{m(k)}_x\,{l(z)}_{a-x}\,dx$$

Following @Coresh1988 we can express the fertility part in a multiplicative way with variability in the level but not in the shape, as ${m(k)}_x = F_\beta\,k\,r_x$, where $F_\beta$ is the baseline cumulated fertility, $K$ is a random variable with mean 1 that allows variability between groups and $r_x$ is the fertility structure by age ($\int_{0}^{\beta}{r_x}=1$). Frailty part can be thought in a cohort effect way, also in the multiplicative assumption as @Vaupel2014, with $l(z)_x=e^{-H_{x}Z}=l_x^Z$, with baseline hazard $\mu$. Replacing conveniently the last two expressions and considering the joint distribution $f_{kz}=f_{z/k} \,f_k$, the mean $\overline{CS}_a$ would be:
...
The way to chain dependence is convenient to the expected herarchy $f_{zk}=f_{k|z}f_z$: groups with higher descendant´s mortality would adjust their fertility level, with positive correlation. Isolating one age and defining $f_{kz}$ we can explore effects in $CS_a$.
