...
We now consider the consequences of a change $\delta$ in mortality in the range $[0,a-\alpha]$, where $\alpha$ is the start age of fertility risk. A discrete approximate of the effect on $CS_a$ given an absolute change in mortality, considering that $m_{x,\delta}=m_{x}+\delta$ (@wrycza2012), and $l_{a-x} = e^{-\int_{0}^{a-x}{(\mu_t+\delta})dt}$:
...
If the change is only in the age range $\left[0;1\right)$, infant mortality, we can start by splitting the integral^[$a>1$]:
...
Expressing $\int_{0}^{a-1} m_{x} l_{a-x} dx$ as $CS_a - CS_{a-1,a}$, and finally dividing by $CS_a$: 
...
Affects proportionally for the range extension of 1, less $P_b$ (if we assume uniform distribution of deaths and births in the first year) times $\frac{CS_{a-1,a}}{CS_a}$ the portion of cumulated alive daughters in the previous year, weighting how important was the last year of reproduction.
...
In a model with heterogeneity, determined at born with a multiplicative effect, $CS_a$ could be interpreted as a conditional expectation with random variables *Y* for fertility heterogeneity and *Z* for frailty.

$$CS_a = \int_{0}^{\infty}f_y \int_{0}^{\infty}f_{z|y}\left[\int_{0}^{\infty}y\,\overline{m}_x\,e^{-\overline{H}_{a-x}\,z}\,dx\right]\,dz\,dy\\
CS_a = \int_{0}^{\infty}f_y \, y \int_{0}^{\infty}f_{z|y}\left[\int_{0}^{\infty}\,\overline{m}_x\,e^{-\overline{H}_{a-x}\,z}\,dx\right]\,dz\,dy
$$
