...
We now consider the consequences of a change $\delta$ in mortality in the range $[0,a-\alpha]$, where $\alpha$ is the start age of fertility risk.
...
Where $H_{CS_a}$ can be interpreted as a temporary entropy constant until age $a-\alpha$ . 
Depending the age structure of the survival daughters $C_{CS_a}(x)=\frac{m_{x}l_{a-x}}{CS_{a}}$,  is the way that is taken each age contribution to the disparity measure $H_{CS_a}=\int_\alpha^a{C_{CS_a}(x)H(x)}$ (@Aburto2019).  

The effect of a relative change $\delta$ will be bigger when the mortality is more disperse in ages where the fertility is very concentrated. For example in \ref{fig:relative_change}, the youngest schedule would have a bigger change in $C_{CS_a}$...


```{r relative_change, echo=F, warning=F, fig.cap="Two different fertility schedules (black lines) and a big hump mortality case (right axis, line in red). In dashed grey lines are the mean age at birth"}
...
To find the effects on daughter survival of adding $\delta$ to the age-specific death rates of daughters, we get the derivative of $dCS_{(a)}^\delta / d\delta$, evaluated near zero (@KeyfitzMath, section 4.3):
...
Calling $C_{CS_a}(x)=\frac{m_{x}l_{a-x}}{CS_{a}}$ the weighting function of the succesful fertility pattern (onset distribution of survival daughters), and making an analogy with the classical entropy function $H$ where the weights are just the survival function (@KeyfitzMath), we can define this information function $H_{CS_a}=\int_\alpha^a{C_{CS_a}H(x)}$, and get:
...
This section proposes three additional measures related to the child survival measure for a mother aged $a$. 

*Mean Time Spent with Lost* ($MTSL$) is an absolute measure of the expected time with a death daughter that lives a mother, which can be expressed in terms of a temporary expected lost years measure $e^\dagger$: 
...
It is reasonable to think that mothers relates these lost years with the expected time with live daughters, getting the *intensity* of her losts (*Intensity Time Lost* ($ITL$)). This makes possible also to compare between fertility regimes:
...
Different populations could have different mean age at lost ($MAL$) of mothers with death child:
