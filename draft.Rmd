...
A discrete approximate of the effect on $CS_a$ given an absolute change in mortality $m_{x,\delta}=m_{x}+\delta$ is (@wrycza2012):
...
The effect of a relative change $\delta$ will be bigger when the mortality is more disperse in ages where the fertility is more concentrated. For example in \ref{fig:relative_change}, with a big hump in first adult ages, the youngest schedule would have a bigger change in than the older one in $C_{CS_a}$.


```{r relative_change, echo=F, fig.align='center', warning=F, out.width = '70%', fig.cap="Two different fertility schedules (black lines) and a big hump mortality case (right axis, line in red). In dashed grey lines are the mean age at birth for each pattern."}
...
It is reasonable to think that mothers relates these lost years with the expected time with live daughters, getting the *intensity* of her losts (*Intensity Time Lost* ($ITL$)): ratio between expected time with "lost"" and expected time with "life". This makes possible also to compare between fertility regimes:
...
Different populations could have different mean age at lost ($MAL_a$) for mothers aged $a$ with at least a death child:
...
$K_a$ is the mean age at birth for womens aged $a$, $MAD_{a-x}$ refers to the mean age at death for those newborns who dies before $a-x$, $TFR_a$ is the cumulated fertility for a women aged a, and $f_{x,a}$ is the cumulative age distribution of fertility until age $a$.

Equation \ref{eq:MAL} can be derived by starting with the mother age $x+t$ at each death child age at death $t$, weighted by the fertility and survival function:
