...
Consider a stable female population with fertility rates $m_{x}$ and force of mortality $\mu_{x}$, where the expected number of surviving children to a woman aged $a$, 
...
This was developed in @goodman1974, in the "counting method" approach, being the net reproduction rate $R_0$ limited to $a$, or the (not complete) ratio between generations (multiplying and dividing by the mother generation $B$). 

This can be affected by a change of $\delta$ in mortality in the range $[0,a-\alpha]$, where $\alpha$ is the start age of fertility risk  (@goodman1974). Following @wrycza2012, this change can be generalized as a function of $\delta$, being an absolute change  $m_{x,\delta}=m_{x}+\delta$ and a relative one $m_{x,\delta}=m_{x}(1+\delta)$. In the first case, a discrete approximate effect is:
